$port = if (Test-Path 'D:\Python codes\hmccontroller\.dual-graph\mcp_port') { Get-Content 'D:\Python codes\hmccontroller\.dual-graph\mcp_port' } else { '8081' }
try {
...
$ctxFile = 'D:\Python codes\hmccontroller\CONTEXT.md'
if (Test-Path $ctxFile) { Write-Output ""; Write-Output "=== CONTEXT.md ==="; Get-Content $ctxFile -Raw; Write-Output "=== end CONTEXT.md ===" }
$storeFile = 'D:\Python codes\hmccontroller\.dual-graph\context-store.json'
